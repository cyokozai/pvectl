...
以下は Contributor Covenant v2.1 の**公式日本語訳**である。訳文は
[Contributor Covenant 日本語版](https://www.contributor-covenant.org/ja/version/2/1/code_of_conduct/)
（原文: [EthicalSource/contributor_covenant](https://github.com/EthicalSource/contributor_covenant) の
`content/version/2/1/code_of_conduct.ja.md`）をそのまま採用し、報告窓口のみ pvectl のものに差し替えている。
拘束力を持つのは英語版（[CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md)）であり、解釈が分かれた場合は英語版を優先する。
...
虐待的、嫌がらせ、またはその他の許容できない行動の事例は、執行を担当するコミュニティリーダーに対して、メンテナの GitHub プロフィール [https://github.com/cyokozai](https://github.com/cyokozai) に記載された連絡先（`[連絡方法を挿入]`）で報告される場合があります。メンテナに非公開で連絡する手段がない場合は、詳細を書かずに「行動規範に関する報告がある」とだけ記した GitHub issue を立ててください。すべての苦情は迅速かつ公正にレビューおよび調査されます。
